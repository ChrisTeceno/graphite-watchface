...
- **Tap the date** to open the calendar, or **the weather** to open the weather app.
...
- **`<Launch>` accepts a package name, but there is no WEATHER system shortcut.** The constants cover ALARM, BATTERY_STATUS, CALENDAR, HEALTH_HEART_RATE, MESSAGE, MUSIC_PLAYER, PHONE and SETTINGS. Anything else needs a package name or a full component, so the weather tap hardcodes `com.google.android.apps.weather` and would do nothing on a non-Pixel watch. Wrapping the cell in a `<Group>` puts one `<Launch>` over the icon, the temperature and the gap between them.
- **Taps only register on a properly awake screen, and a dozing watch reports success while silently swallowing them.** `adb shell input tap` against a dimmed face does nothing and logs nothing, which reads exactly like a broken `<Launch>`. Every tap test needs `input keyevent 224` first, then the HOME intent, then a settle. This produced a string of false negatives here and nearly had a working feature reverted as broken.
